...
template RangeProof(n) {
    ...
    // if in range: high.out == low.out == 1, so we return 1 else one "out" is 0 and 0 multiplied by something is 0
...
template sudoku() {
    ...
    component rangeCheck[9][9][2];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangeCheck[i][j][0] = RangeProof(252);
            rangeCheck[i][j][0].in <== puzzle[i][j];
            rangeCheck[i][j][0].range[0] <== 0;
            rangeCheck[i][j][0].range[1] <== 9;

            // needs to be 1 if in range
            rangeCheck[i][j][0].out === 1;

            rangeCheck[i][j][1] = RangeProof(252);
            rangeCheck[i][j][1].in <== solution[i][j];
            rangeCheck[i][j][1].range[0] <== 0;
            rangeCheck[i][j][1].range[1] <== 9;

            // needs to be 1 if in range
            rangeCheck[i][j][1].out === 1;
